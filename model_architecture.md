...
여기서 모듈화는 파일이나 클래스 이름만 나누는 것을 뜻하지 않습니다. 한 클래스가 요청 검증, 모델 호출, 결과 판정, 데이터 저장까지 모두 처리하는 God Class·God Object를 만들지 않도록 **다른 책임과 데이터 수정 권한을 분리**합니다. 초기에는 한 프로세스로 배포하더라도 코드에서는 각 모듈의 공개 인터페이스를 통해서만 협력하게 합니다.
...
외부 영수증 21장 최초 평가의 종합점수는 87.8%였지만 고정 설정으로 세 번 반복한 현재 기준은 정밀도 95.1%, 재현율 73.6%, 종합점수 83.0%였습니다. 전체 95% 완료시간도 18.44초였습니다. 현재 결과는 자동 등록 승인 기준을 통과하지 못했으므로, 아키텍처가 완성되더라도 별도 촬영 평가 자료와 실제 EC2 측정을 통과하기 전에는 자동 확정 범위를 확대하지 않습니다.
...
| 데이터 소유권 분리 | 재고 수정은 서비스 API만, 모델 결과와 추적 정보 저장은 AI Gateway만, 상품 사전 갱신은 정규화 모듈의 관리 기능만 수행합니다. |
...
| 분석 결과·추적 정보 | 서비스 API, 평가 모듈 | AI Gateway의 결과 저장 기능 | 모델별 출력과 사용자 확정값을 분리해 보존 |
...
데이터베이스 계정도 권한에 맞게 나눕니다. AI 작업자는 필요한 상품 자료를 읽고 분석 결과만 기록할 수 있으며 사용자 재고 테이블에 대한 수정 권한을 갖지 않습니다. 추천 모듈은 재고와 레시피를 읽을 수 있지만 상품 사전과 사용자 재고를 수정하지 않습니다.
...
작업자에게 사용자 인증 토큰이나 냉장고 수정 권한을 전달하지 않습니다. 작업자는 `analysisId`의 결과만 기록하고 재고 반영 여부는 서비스 API가 사용자 확인 후 결정합니다.
...
이 단계는 초기 출시부터 적용하지 않습니다. 큐 대기시간, 메모리 사용량, 요청 비율과 장애 영향 범위를 측정해 한 프로세스로 감당하기 어렵다는 근거가 생겼을 때 진행합니다.
