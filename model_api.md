...
```mermaid
flowchart LR
    U[PWA 사용자] -->|Presigned URL 요청| B[서비스 백엔드]
    B -->|Presigned URL 발급| U
    U -->|이미지 직접 업로드| S[(S3 이미지 저장소)]
    U -->|업로드 완료·분석 요청| B
    ...
    E -- 아니오 --> Q[분석 작업 큐]
    Q --> P[EC2 CPU 이미지 전처리]
    P --> M[MobileNetV3·PP-OCRv5·바코드]
    M --> D[MySQL 상품·별칭·비식품 사전]
    D --> C{사전으로 확정 가능?}
    C -- 예 --> K
    C -- 모르는 영수증 상품 --> G[Ollama·Gemma CPU 보완]
    C -- 실물·자체 처리 실패 --> X[예산 제한 외부 AI API]
    G --> K[원문 일치·중복·날짜 결정 규칙]
    X --> K
    K --> R
    R --> B
    B -->|확인·수정 화면| U
...
브라우저 기기 성능에 따라 결과가 달라지지 않도록 모든 AI 추론은 EC2에서 수행합니다. PWA의 이미지 리사이즈·압축은 추론이 아니라 전송량을 줄이기 위한 입력 준비 단계입니다.

초기 출시에서는 AWS 그래픽 처리장치를 사용하지 않습니다. EC2 `m7i.xlarge` 중앙처리장치에서 AI 작업자 한 개가 대기열을 순서대로 처리하며, Qwen-VL과 vLLM은 운영 구성에 포함하지 않습니다. 자체 경로가 품질 또는 처리시간 기준을 충족하지 못한 요청만 월 12만 원 한도에서 외부 인공지능 API로 보완합니다.
...
2단계의 외부 영수증 21장 평가에서 상품행 정밀도는 95.6%였지만 재현율 81.1%, 종합점수 87.8%, 전체 95% 완료시간 18.44초였습니다. 목표인 상품행 재현율·종합점수 90%와 긴 영수증 95% 완료시간 12초를 충족하지 못했으므로 현재 파이프라인을 자동 등록 모델로 승인하지 않습니다.
