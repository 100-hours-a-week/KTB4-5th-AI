...
| 영역 | 담당 기능 | 담당하지 않는 기능 |
| --- | --- | --- |
| PWA | 촬영·파일 선택, 이미지 리사이즈·압축, S3 업로드, 작업 상태 조회, 결과 확인·수정 | 분류, OCR, VLM, RAG, 추천 랭킹 |
| 서비스 백엔드 | 인증·권한, Presigned URL 발급, 분석 작업 생성, 재고 저장, 결과 조회 | 모델 추론 |
| EC2 AI 워커 | 전처리, 이미지 유형·화질 분류, OCR, 바코드, Qwen-VL, 결과 정규화·상태 판정 | 사용자 인증과 냉장고 권한 판단 |
| EC2 추천 워커 | RAG query 임베딩, 레시피 검색, 임박도·용량·취향 랭킹 | 이미지 업로드와 재고 원본 변경 |
